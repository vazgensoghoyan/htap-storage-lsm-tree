...
void ColumnSSTableBuilder::flush_block() {
    ...
    std::vector<read::sstable::NumericBlockStats> logical_block_stats;
    ...
    for (size_t ci = 0; ci < col_builders_.size(); ++ci) {
        ...
        for (auto& stats : result.numeric_stats) {
            logical_block_stats.push_back(std::move(stats));
        }
    }
...
void ColumnSSTableBuilder::write_stats_file() {
    ...
    if (!stats_file) {
        throw std::runtime_error("ColumnSSTableBuilder: failed to open stats file: " +
                                 paths_.stats().string());
    }
